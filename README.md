...
🔹 사용한 데이터셋
사람 작성 텍스트: 국내 초·중·고 학생들이 실제로 작성한 에세이(찬성반대, 글짓기, 대안제시, 설명글, 주장 등)를 수집하였습니다. 학년별로 글의 길이, 어휘 수준, 문체에서 차이를 보입니다.
AI 생성 텍스트: 동일한 주제 또는 유사 맥락에서 생성형 AI(ChatGPT, gemini 등)를 활용해 생성한 텍스트를 수집하였으며, 사람이 작성한 글과 최대한 유사한 주제와 스타일로 조정하였습니다.
...
**AI 텍스트의 평균 길이(661.5)**는 **Human 텍스트(632.5)**보다 소폭 길다.
...
🔹 프로젝트 진행 중 겪은 한계
데이터 다양성의 부족 :
수집한 사람 작성 텍스트는 대부분 초·중·고 학생의 에세이 형식으로, 주제가 ‘나의 꿈’, ‘환경 보호’, ‘가족’ 등 특정 범위에 집중되어 있습니다.
반면 AI 생성 텍스트는 무작위 주제 기반으로 생성되어, 유사한 주제를 놓고 비교하는 정밀한 판별에는 어려움이 있었습니다.
예시 :
사람 작성: “저는 나중에 훌륭한 선생님이 되고 싶습니다. 아이들을 잘 가르치고 싶기 때문입니다.”
AI 작성: “교사가 되는 것은 지식 전달 이상의 의미를 지니며, 사회적 책임과 윤리적 자세가 요구된다.”

AI 생성기 종류의 제한 :
AI 텍스트는 주로 GPT-3.5 기반 ChatGPT에서 생성된 것으로, 다른 한국어 기반 생성기(예: KoAlpaca, Mistral, Claude 등)의 문체나 패턴은 고려하지 못했습니다.
다양한 AI 모델의 문체 특성을 포괄하지 못해 실제 적용 시 범용성이 떨어질 가능성이 있습니다.
복잡하거나 비형식적인 문장에 대한 일반화가 어려우며 창의적인 표현이나 오타, 줄임말 등을 포함한 문장은 AI 생성 여부를 판단하기 까다로운 경우가 있었습니다.
예시:
“ㄹㅇ 개힘들엇음 ㅋㅋㅋ” → 사람 작성 가능성이 높지만, AI 생성 모델에서도 모사 가능
“이러한 고찰은 윤리적 숙고를 동반한다.” → 고학년 학생이 썼더라도 AI로 오탐 가능성 있음

🔹 개선이 필요한 부분 및 추가 보완 아이디어
다양한 AI 생성기 반영
Claude, KoAlpaca, KoGPT 등의 다양한 생성 모델을 활용하여 동일한 주제에 대해 텍스트를 생성하고, 이를 학습 데이터에 포함함으로써 문체 다양성을 반영할 예정입니다.
예시: ‘기후 변화’ 주제로 GPT-3.5와 KoAlpaca가 작성한 결과 비교

형식적 피처 기반 보조 모델 설계
문장 평균 길이, 조사 사용 빈도, 단어 다양도(type-token ratio), 맞춤법 오류 수 등 언어적 스타일 지표를 정량화하여 부가 입력값으로 활용하는 방법도 고려하고 있습니다.
다중 분류 또는 연속형 예측 모델 전환
단순히 AI/사람 여부를 이진 분류하는 것이 아닌, ‘AI 생성 가능성 점수’를 0~1 사이로 예측하거나, GPT, Claude, KoGPT 등 모델별 분류로 확장하는 것을 검토하고 있습니다.
세분화된 라벨링 및 연령 고려
특히 초등학생이 작성한 글은 문체가 간단하고 반복적이므로, 학년별 라벨링 및 사람 텍스트 내에서도 분류 기준을 강화하는 방식으로 성능을 개선할 수 있습니다.
예시:
