...
- **복잡한 언어 현상 단순화**
  - 자연어 문장은 단순히 ‘Human’ 또는 ‘AI’로 명확히 나뉘기 어려움.
  - AI가 생성했지만 매우 자연스러운 문장이나, 인간 작성이지만 기계적이고 어색한 문장 등 경계가 모호한 경우가 많음.
  - 이진 분류는 이러한 미묘한 스펙트럼을 반영하지 못해 정보 손실이 발생함.

- **중간 형태의 문장 처리 한계**
  - AI가 부분적으로 편집한 인간 문장, 혹은 인간과 AI가 협업한 문장 등 중간 상태를 구분하지 못함.
  - 강제적으로 두 클래스 중 하나에 할당하기 때문에 분류 오류가 증가할 수 있음.

- **다양한 AI 모델 구분 불가**
  - 여러 LLM(예: GPT-4o, Claude, Qwen 등)이 혼재한 데이터셋에서, 단순히 ‘AI’ 클래스로만 묶으면 모델별 특성이나 스타일 차이를 반영할 수 없음.
  - 세분화된 분석이나 맞춤형 대응이 어렵다는 한계 존재.

- **실제 활용과 평가 목적 불일치 가능성**
  - 단순히 AI/인간 구분에만 집중하면, 문장 품질 평가, 감정 분석, 내용 이해 등 다양한 NLP 태스크에 부적합할 수 있음.
  - 이진 분류가 목적에 비해 지나치게 제한적인 시각을 제공함.

- **변화하는 AI 생성 특성 반영 어려움**
  - AI 생성 문장은 시간이 지날수록 발전하고 변화함.
  - 고정된 이진 라벨링은 최신 생성 스타일을 포착하기 어렵고, 모델의 적응력을 저해할 수 있음.

---

이러한 한계들 때문에, 단순한 이진 분류 외에도 다중 클래스 분류, 생성자 식별, 품질 점수화 등의 보다 정교한 분류 체계 도입이 필요할 수 있습니다.
